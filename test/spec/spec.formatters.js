...
function testStringFormatter(name, values) {
    describe(name, function () {
        var fn = Formatters[name];
        it('passes through non-string', function () {
            fn(0).should.equal(0);
        ...
        Object.keys(values).forEach(function (input) {
            var output = values[input];
            it('formats "' + input + '"" to "' + output + '"', function () {
...
describe('Formatters', function () {
    ...
    describe('boolean', function () {
        it('formats inputs correctly', function () {
