...
_.extend(Form.prototype, {
    ...
    _locals: function (req, res, callback) {
        _.extend(res.locals, {
            ...
            action: this.absoluteUrl(req, req.path)
        });
    ...
    getNextStep: function (req, res) {
        ...
        next = forks.reduce(function (result, value) {
            return evalCondition(value.condition) ?
                value.target :
                result;
        }, next);

        return this.absoluteUrl(req, next);
    },
    getErrorStep: function (err, req) {
        ...
        return this.absoluteUrl(req, redirect);
    },
