...
var _ = require('underscore'),
    ...
    dataFormatter = require('./formatters'),
    dataValidator = require('./validators');
...
_.extend(Form.prototype, {
    ...
    _process: function (req, res, callback) {
        req.form = { values: {} };
        var formatter = dataFormatter(this.options.fields, ['trim', 'singlespaces', 'hyphens']);
    ...
    isValidationError: function (err) {
        return !_.isEmpty(err) &&  _.all(err, function (e) { return e instanceof Form.Error; });
    },
    errorHandler: function (err, req, res, callback) {
        if (err.code === 'MISSING_PREREQ') {
            this.emit('prerequisite-error', req, res);
...
Form.Error = require('./validation/error');
